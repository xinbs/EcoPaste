...
router.post("/upload", authenticateToken, async (req, res) => {
	try {
		...
		logger.info(
			`用户 ${userId} 设备 ${deviceId} 上传 ${results.length} 个项目`,
		);
			...
			sync: {
				...
				successRate:
					syncStats.total_syncs > 0
						? (
								((syncStats.successful_syncs || 0) / syncStats.total_syncs) *
								100
							).toFixed(2)
						: "0.00",
			},
...
router.get("/search", authenticateToken, async (req, res) => {
	try {
		...
		let whereClause =
			"ci.user_id = ? AND ci.is_deleted = 0 AND ci.content LIKE ?";
		const params = [userId, `%${q.trim()}%`];
	...
	try {
       ...
       WHERE ${whereClause}`,
		...
		res.json({
			...
			syncTime: new Date().toISOString(),
			hasMore:
				Number.parseInt(offset) + Number.parseInt(limit) < countResult.total,
		});
	} catch (error) {
...
router.post("/push", authenticateToken, async (req, res) => {
	try {
		...
		logger.info(
			`用户 ${userId} 设备 ${deviceId} 推送 ${results.length} 个项目`,
		);
	...
	try {
		...
		await dbRun(
			...
			[
				...
				"force_sync",
				"started",
				JSON.stringify({ type: "force_sync_all" }),
			],
		);
       ...
       WHERE user_id = ? AND is_deleted = 0`,
		...
		await dbRun(
			...
			[
				"completed",
				JSON.stringify({ type: "force_sync_all", stats }),
				userId,
				deviceId,
				"force_sync",
				"started",
		...
		logger.info(
			`用户 ${userId} 设备 ${deviceId} 执行强制全量同步，处理 ${stats.total_items} 个项目`,
		);
...
router.post(
	...
	async (req, res) => {
		try {
			...
			logger.info(
				`用户 ${userId} 解决冲突 ${conflictId}，策略: ${resolution.strategy}`,
			);
       ...
       WHERE user_id = ? AND is_deleted = 0`,
		...
		await dbRun(
			...
			[
				...
				"backup",
				"completed",
				JSON.stringify({ name: backupName, ...stats }),
			],
		);
	...
	try {
       ...
       ORDER BY ci.created_at DESC`,
		...
		if (format === "csv") {
			...
			res.setHeader(
				"Content-Disposition",
				'attachment; filename="clipboard_export.csv"',
			);
			...
			res.setHeader(
				"Content-Disposition",
				'attachment; filename="clipboard_export.json"',
			);
			res.json(exportData);
		}
	...
	try {
		...
		await dbRun(
			...
			[
				...
				"import",
				"completed",
				JSON.stringify({ format, itemCount: importData.items?.length || 0 }),
			],
		);
