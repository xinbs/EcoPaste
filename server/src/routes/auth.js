...
router.post("/register", async (req, res) => {
	try {
		const { username, password, email, deviceName, deviceType, platform } =
			req.body;
		...
		// 生成JWT token
		const token = jwt.sign({ userId, deviceId, username }, JWT_SECRET, {
...
router.post("/refresh", async (req, res) => {
	try {
		...
		// 生成新token
		const newToken = jwt.sign({ userId, deviceId, username }, JWT_SECRET, {
			expiresIn: JWT_EXPIRES_IN,
		});
