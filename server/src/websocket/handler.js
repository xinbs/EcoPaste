...
// WebSocket连接处理
export function handleWebSocketConnection(ws, _req) {
	...
	ws.on("close", (code, reason) => {
		...
		logger.info(
			`WebSocket连接关闭: ${connectionId}, 代码: ${code}, 原因: ${reason}`,
		);
	});
	...
	// 处理同步请求
	async function handleSync(ws, data, _connectionId) {
		if (!userId || !deviceId) {
			sendError(ws, "未认证");
			return;
		}
		...
		try {
         ...
         ORDER BY ci.updated_at ASC`,
			...
			logger.debug(
				`为用户 ${userId} 设备 ${deviceId} 同步了 ${updates.length} 条记录`,
			);
		} catch (error) {
	...
	// 处理剪贴板更新
	async function handleClipboardUpdate(ws, data, _connectionId) {
		if (!userId || !deviceId) {
			sendError(ws, "未认证");
			return;
		}
